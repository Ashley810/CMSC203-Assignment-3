...
public class CryptoManager { 
    ...
    public static String vigenereEncryption(String plainText, String key) {
         //First check if the text is inside the range
    	if (!isStringInBounds(plainText)) {
    	    ...
    	    for (int i = 0; i < plainText.length(); i++) {
    	    	...
    	    	// Repeat the key so it lines up with the plaintext
    	        char keyChar = key.charAt(i % key.length());

    	        // Find the index of the plaintext character inside the 64‑alphabet
    	        // I start at -1 so I know I haven't found it yet
    	        int plainIndex = -1;
    	        for (int j = 0; j < ALPHABET64.length(); j++) {
    	            if (ALPHABET64.charAt(j) == plainChar) {
    	                plainIndex = j;
    	                break; // stop once we find it
    	            }
    	        }

    	     // Do the same thing for the key character
    	        int keyIndex = -1;
    	        for (int j = 0; j < ALPHABET64.length(); j++) {
    	            if (ALPHABET64.charAt(j) == keyChar) {
    	                keyIndex = j;
    	                break;
    	            }
    	        }
    	        
    	     // Vigenere formula: add the two indexes and wrap around using mod 64
    	        int encryptedIndex = (plainIndex + keyIndex) % 64;
    	        
    	     // Convert the index back into a character from the alphabet
    	        encryptedText += ALPHABET64.charAt(encryptedIndex);
    	    }
     ...
     // Loop through each encrypted character
        for (int i = 0; i < encryptedText.length(); i++) {
         ...
         // Find the index of the encrypted character
            ...
            for (int j = 0; j < ALPHABET64.length(); j++) {
                if (ALPHABET64.charAt(j) == encryptedChar) {
                    encryptedIndex = j;
                    break;
                }
            }
         ...
         // Find the index of the key character
            ...
            // Reverse the Vigenere formula: subtract the key index
            // I add 64 before mod so it never goes negative
            int decryptedIndex = (encryptedIndex - keyIndex + 64) % 64;

            decryptedText += ALPHABET64.charAt(decryptedIndex);
        }
    ...
    public static String caesarEncryption(String plainText, int key) {
        // First, check if all characters in plainText are within the allowed range
        if (!isStringInBounds(plainText)) {
            // If any character is out of range, return an error message
            return "The selected string is not in bounds, Try again.";
        }
     ...
     // Loop through each character in the plaintext
        for (int i = 0; i < plainText.length(); i++) {
         ...
         // Find the index of the plaintext character in the 64 alphabet
            int plainIndex = -1;
            for (int j = 0; j < ALPHABET64.length(); j++) {
                if (ALPHABET64.charAt(j) == plainChar) {
                    plainIndex = j;
         ...
         // Caesar formula: shift by the key and wrap around
            int encryptedIndex = (plainIndex + key) % 64;
    ...
    public static String caesarDecryption(String encryptedText, int key) {
     ...
     // Loop through each encrypted character
        for (int i = 0; i < encryptedText.length(); i++) {
         ...
         // Find the index of the encrypted character in the 64 alphabet
            int encryptedIndex = -1;
            for (int j = 0; j < ALPHABET64.length(); j++) {
                if (ALPHABET64.charAt(j) == encryptedChar) {
                    encryptedIndex = j;
            ...
            int decryptedIndex = (encryptedIndex - key + 64) % 64;
